...
export default function AifeilerBookReader() {
  ...
  const pageImage = <Image key={page} src={asset(bookImage(page))} alt={`《遺落在風中的信》第 ${page} 頁`} width={720} height={1023} sizes="(min-width: 768px) 620px, 94vw" className="h-auto max-h-[76svh] w-auto max-w-full object-contain" priority={page <= 2} />;
    ...
    <section id="aifeiler-book" className="bg-[#efe4d2] px-3 py-14 text-[#31294e] sm:px-8 sm:py-20">
      <div className="mx-auto max-w-5xl">
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-base leading-8 text-[#31294e]/70 sm:text-lg">由李宛容 Ivy 創作的療癒圖文書。完整保留封面、書封內頁、152 頁正文、作者介紹與封底，在網站裡即可直接翻閱。</p>
        </div>
        ...
        <div className="mx-auto mt-7 max-w-3xl overflow-hidden rounded-[1.75rem] border border-[#9b7132]/35 bg-[#17143d] shadow-2xl">
          ...
          <div className="flex min-h-[420px] items-center justify-center bg-[#0d0b24] p-3 sm:p-6">{pageImage}</div>
          <div className="border-t border-white/10 px-4 py-4 sm:px-6">
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
              <button type="button" onClick={() => goTo(page - 1)} disabled={page === 1} className="justify-self-start rounded-full border border-[#f4d892]/40 px-3 py-2.5 text-sm font-bold text-[#f4d892] disabled:opacity-30">← 上一頁</button>
              <p className="font-serif text-sm font-bold text-white"><span className="text-xl text-[#f4d892]">{page}</span> / {TOTAL}</p>
              <button type="button" onClick={() => goTo(page + 1)} disabled={page === TOTAL} className="justify-self-end rounded-full bg-[#d6a94f] px-3 py-2.5 text-sm font-bold text-[#17143d] disabled:opacity-30">下一頁 →</button>
            </div>
          </div>
        </div>
      </div>

      {expanded && <div role="dialog" aria-modal="true" aria-label={`《遺落在風中的信》第 ${page} 頁`} className="fixed inset-0 z-[110] flex items-center justify-center bg-black/95 p-2 sm:p-6" onClick={() => setExpanded(false)}><button type="button" onClick={() => setExpanded(false)} aria-label="關閉電子書放大畫面" className="absolute right-4 top-4 z-10 flex h-12 w-12 items-center justify-center rounded-full bg-[#d6a94f] text-3xl text-[#17143d]">×</button>{pageImage}</div>}
    </section>
  );
}
