...
type Star = { top: string; left: string; size: number; delay: string; duration: string; gold?: boolean };

const STARS: Star[] = [
  { top: "4%", left: "12%", size: 2, delay: "0s", duration: "3.2s" },
  { top: "8%", left: "42%", size: 5, delay: "0.6s", duration: "4s", gold: true },
  ...
  { top: "22%", left: "6%", size: 4, delay: "1.8s", duration: "3.8s", gold: true },
  ...
  { top: "31%", left: "48%", size: 6, delay: "2.4s", duration: "4.4s", gold: true },
  ...
  { top: "49%", left: "62%", size: 4, delay: "0.7s", duration: "3.9s", gold: true },
  ...
  { top: "67%", left: "14%", size: 5, delay: "1.5s", duration: "4.1s", gold: true },
  ...
  { top: "84%", left: "58%", size: 4, delay: "0.8s", duration: "4s", gold: true },
...
export default function StarField() {
  return (
    <div
      aria-hidden="true"
      className="pointer-events-none fixed left-1/2 top-0 z-[20] h-full w-full max-w-[430px] -translate-x-1/2 overflow-hidden"
    >
      {STARS.map((s, i) => (
        <span
          key={i}
          className={`animate-twinkle absolute rounded-full ${s.gold ? "bg-gold-light" : "bg-paper"}`}
          style={{
            top: s.top,
            left: s.left,
            width: `${s.size}px`,
            height: `${s.size}px`,
            boxShadow: `0 0 ${s.size * 2.2}px ${s.size * 0.9}px ${
              s.gold ? "rgba(236,206,143,0.55)" : "rgba(250,246,238,0.5)"
            }`,
