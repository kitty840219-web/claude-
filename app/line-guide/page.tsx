...
function StepList({ steps }: { steps: { src: string; label: string }[] }) {
  return (
    <div className="space-y-4">
      {steps.map((s) => (
        <figure key={s.src} className="overflow-hidden rounded-2xl border border-gold/15 bg-night-light/20 shadow-card">
          <div className="relative aspect-square w-full">
            <Image src={asset(s.src)} alt={s.label} fill className="object-cover" sizes="430px" />
          </div>
          <figcaption className="px-4 py-3 text-xs font-semibold text-paper/70">{s.label}</figcaption>
        </figure>
      ))}
    </div>
  );
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-night-dark px-4 pb-10 pt-6 sm:px-6">
        ...
        <div className="relative mx-auto max-w-3xl">
          <p className="mb-4 text-xs font-semibold tracking-[0.25em] text-gold-light">新選單功能教學</p>
          <StepList steps={MENU_STEPS} />
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-night-dark px-4 pb-20 pt-4 sm:px-6">
        <div className="bg-stars pointer-events-none absolute inset-0 opacity-30" />
        <div className="relative mx-auto max-w-3xl">
          <p className="mb-4 text-xs font-semibold tracking-[0.25em] text-gold-light">大眾占卜／心理測驗 關鍵字領取教學</p>
          <StepList steps={KEYWORD_STEPS} />
